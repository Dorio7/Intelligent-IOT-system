...
static const GUI_WIDGET_CREATE_INFO _aDialogCreateClock[] = {
  { FRAMEWIN_CreateIndirect, "Clock",                  0,     0,   0,   800, 480, 0, 0x0,  0 },
  ...
  { DROPDOWN_CreateIndirect, "Date_month",  GUI_ID_DROPDOWN0, 575, 120, 120, 25,  0, 0x0,  0 },
  { CHECKBOX_CreateIndirect, "Date_Check",  GUI_ID_CHECK0,    480, 120, 25,  25,  0, 0x0,  0 },
  { CHECKBOX_CreateIndirect, "Time_Check",  GUI_ID_CHECK1,    480, 190, 25,  25,  0, 0x0,  0 },
  { BUTTON_CreateIndirect,   "Apply",       GUI_ID_BUTTON0,   570, 300, 80,  30,  0, 0x0,  0 },
  { EDIT_CreateIndirect,     "Date_day",    GUI_ID_EDIT0,     525, 120, 35,  25,  0, 0x2,  0 },
  { EDIT_CreateIndirect,     "Date_year",   GUI_ID_EDIT1,     710, 120, 60,  25,  0, 0x4,  0 },
  { EDIT_CreateIndirect,     "Time_hour",   GUI_ID_EDIT2,     525, 190, 35,  25,  0, 0x2,  0 },
  { EDIT_CreateIndirect,     "Time_min",    GUI_ID_EDIT3,     595, 190, 35,  25,  0, 0x2,  0 },
  { EDIT_CreateIndirect,     "Time_sec",    GUI_ID_EDIT4,     665, 190, 35,  25,  0, 0x2,  0 },
...
static void _cbDialogClock(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:    
    ...
    CALENDAR_Create(pMsg->hWin, 20, 120, drv_clock.Date_year+2000, drv_clock.Date_month, drv_clock.Date_day, 1, GUI_ID_CALENDAR0, WM_CF_SHOW);
    // USER END
    break;
...
//          CALENDAR_SetSel(hItem, &current_date); 
  ...
  case WM_PAINT:
    ...
    GUI_DrawRect(460,90,780,400);
  
    GUI_SetColor(GUI_WHITE); 
    GUI_SetFont(GUI_FONT_32B_ASCII);
    GUI_DispCharAt(':',572,185);
    GUI_DispCharAt(':',642,185);
  
    GUI_SetColor(GUI_LIGHTGREEN);
    GUI_SetFont(&XBF_XINSONGTI19);
    GUI_DispStringAt(UTF8_DATE,525,100); 
    GUI_DispStringAt(UTF8_TIME,525,170);       
    break;
  case WM_TIMER:
    RTC_GetTime(RTC_Format_BIN, &rtc_time.RTC_Time);
    RTC_GetDate(RTC_Format_BIN, &rtc_time.RTC_Date);
...
{
  WM_HWIN hWin;
  OS_INFO("ClockDLG create\n");  
  if(bsp_result&BSP_RTC)
  {
    if(RTC_CheckAndConfig(&rtc_time,0)==1)  
      bsp_result |=BSP_RTC;
    else
      bsp_result &=~BSP_RTC;
  }
  hWin = GUI_CreateDialogBox(_aDialogCreateClock, GUI_COUNTOF(_aDialogCreateClock), _cbDialogClock, HDTWIN, 0, 0);
