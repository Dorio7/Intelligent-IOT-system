...
*/
#if   LCD_NUM_LAYERS == 1
  static LTDC_Layer_TypeDef       * _apLayer[]        = { LTDC_Layer1};
  static const U32                  _aAddr[]          = { LCD_FRAME_BUFFER};
#elif LCD_NUM_LAYERS == 2
  static LTDC_Layer_TypeDef       * _apLayer[]        = { LTDC_Layer1, LTDC_Layer2 };
  static const U32                  _aAddr[]          = { LCD_FRAME_BUFFER, LCD_FRAME_BUFFER + XSIZE_PHYS * YSIZE_PHYS * LCD_BUFER_OFFSET * NUM_VSCREENS * NUM_BUFFERS};
  //static const U32                  _aAddr[]          = { LCD_FRAME_BUFFER, LCD_FRAME_BUFFER + XSIZE_PHYS * YSIZE_PHYS * sizeof(U32) * NUM_VSCREENS * NUM_BUFFERS};
#endif
static int                        _aPendingBuffer[LCD_NUM_LAYERS];
static int                        _aBufferIndex[LCD_NUM_LAYERS];
static int                        _axSize[LCD_NUM_LAYERS];
static int                        _aySize[LCD_NUM_LAYERS];
static int                        _aBytesPerPixels[LCD_NUM_LAYERS];
...
#elif LCD_NUM_LAYERS == 2
  if(LayerIndex==0)    
    LTDC_Layer_InitStruct.LTDC_ConstantAlpha = 192;
  else
    LTDC_Layer_InitStruct.LTDC_ConstantAlpha = 255;
#endif
  //
