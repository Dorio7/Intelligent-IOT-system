...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagSW1_OnOff)
    {
        info->event[info->num] = EVENT_SW1_OnOff;
        info->num++;
        dataPoints->valueSW1_OnOff = gizStandardDecompressionValue(SW1_OnOff_BYTEOFFSET,SW1_OnOff_BITOFFSET,SW1_OnOff_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagSW2_OnOff)
    {
        info->event[info->num] = EVENT_SW2_OnOff;
        info->num++;
        dataPoints->valueSW2_OnOff = gizStandardDecompressionValue(SW2_OnOff_BYTEOFFSET,SW2_OnOff_BITOFFSET,SW2_OnOff_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagTime_Off_Minute)
    {
        ...
        info->num++;
        dataPoints->valueTime_Off_Minute = gizX2Y(Time_Off_Minute_RATIO,  Time_Off_Minute_ADDITION, exchangeBytes(issuedData->attrVals.valueTime_Off_Minute));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueSW1_OnOff != cur->valueSW1_OnOff)
    {
        GIZWITS_LOG("valueSW1_OnOff Changed\n");
        ret = 1;
    }
    ...
    if(last->valueSW2_OnOff != cur->valueSW2_OnOff)
    {
        GIZWITS_LOG("valueSW2_OnOff Changed\n");
        ret = 1;
    }
    ...
    if(last->valueLIGHT_BUTTON != cur->valueLIGHT_BUTTON)
    {
        GIZWITS_LOG("valueLIGHT_BUTTON Changed\n");
        ret = 1;
    }
    if(last->valueWORRY_MQ != cur->valueWORRY_MQ)
    {
        GIZWITS_LOG("valueWORRY_MQ Changed\n");
        ret = 1;
    }

    if(last->valueDHT11_wendu != cur->valueDHT11_wendu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueDHT11_wendu Changed\n");
            ret = 1;
        }
    }
    if(last->valueDHT11_shidu != cur->valueDHT11_shidu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueDHT11_shidu Changed\n");
            ret = 1;
        }
    }
    if(last->valuePower1_Consumption != cur->valuePower1_Consumption)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuePower1_Consumption Changed\n");
            ret = 1;
        }
    }
    if(last->valuePower2_Consumption != cur->valuePower2_Consumption)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuePower2_Consumption Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(SW1_OnOff_BYTEOFFSET,SW1_OnOff_BITOFFSET,SW1_OnOff_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSW1_OnOff);
    ...
    gizStandardCompressValue(SW2_OnOff_BYTEOFFSET,SW2_OnOff_BITOFFSET,SW2_OnOff_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSW2_OnOff);
    gizStandardCompressValue(LIGHT_BUTTON_BYTEOFFSET,LIGHT_BUTTON_BITOFFSET,LIGHT_BUTTON_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLIGHT_BUTTON);
    gizStandardCompressValue(WORRY_MQ_BYTEOFFSET,WORRY_MQ_BITOFFSET,WORRY_MQ_LEN,(uint8_t *)devStatusPtr,dataPoints->valueWORRY_MQ);
    ...
    devStatusPtr->valueDHT11_wendu = gizY2X(DHT11_wendu_RATIO,  DHT11_wendu_ADDITION, dataPoints->valueDHT11_wendu); 
    devStatusPtr->valueDHT11_shidu = gizY2X(DHT11_shidu_RATIO,  DHT11_shidu_ADDITION, dataPoints->valueDHT11_shidu); 

    devStatusPtr->valueTime_Off_Minute = exchangeBytes(gizY2X(Time_Off_Minute_RATIO,  Time_Off_Minute_ADDITION, dataPoints->valueTime_Off_Minute)); 
    devStatusPtr->valuePower1_Consumption = exchangeBytes(gizY2XFloat(Power1_Consumption_RATIO,  Power1_Consumption_ADDITION, dataPoints->valuePower1_Consumption));     
    devStatusPtr->valuePower2_Consumption = exchangeBytes(gizY2XFloat(Power2_Consumption_RATIO,  Power2_Consumption_ADDITION, dataPoints->valuePower2_Consumption));     
