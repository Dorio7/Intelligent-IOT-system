...
static void NVIC_2Configuration(void)
{
  NVIC_InitTypeDef NVIC_InitStructure;
  ...
  /* 配置USART为中断源 */
  NVIC_InitStructure.NVIC_IRQChannel = DEBUG2_USART_IRQ;
  /* 抢断优先级*/
...
void USART1_Sned_Char_Buff(u8 buf[],u32 len)  
{  
    u32 i;  
    for(i=0;i<len;i++)  
    USART1_Sned_Char(buf[i]);  
...
void USART2_Sned_Char_Buff(u8 buf[],u32 len)  
{  
    u32 i;  
    for(i=0;i<len;i++)  
    USART2_Sned_Char(buf[i]);  
